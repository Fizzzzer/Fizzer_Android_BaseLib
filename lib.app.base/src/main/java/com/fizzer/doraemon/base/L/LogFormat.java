...
public class LogFormat {
    ...
    public static String formatBorder(String[] segments) {
        ...
        msgBuilder.append(TOP_HORIZONTAL_BORDER).append(LogRootPrinter.lineSeparator);
        for (int i = 0; i < nonNullCount; i++) {
            msgBuilder.append(appendVerticalBorder(nonNullSegments[i]));
            if (i != nonNullCount - 1) {
                msgBuilder.append(LogRootPrinter.lineSeparator).append(DIVIDER_HORIZONTAL_BORDER)
                        .append(LogRootPrinter.lineSeparator);
            } else {
                msgBuilder.append(LogRootPrinter.lineSeparator).append(BOTTOM_HORIZONTAL_BORDER);
            }
        }
    ...
    private static String appendVerticalBorder(String msg) {
        ...
        String[] lines = msg.split(LogRootPrinter.lineSeparator);
        for (int i = 0, N = lines.length; i < N; i++) {
            if (i != 0) {
                borderedMsgBuilder.append(LogRootPrinter.lineSeparator);
            }
            String line = lines[i];
